...
class per_degaton(card_frame.card):
    ...
    def trigger(self, ttype, data, player, active, immediate):
        if globe.DEBUG:
            print("test", self.name, flush=True)
        if trigger.test(not immediate, trigger.DISCARD, self.trigger, player, ttype) \
                and data[0].name == "Punch":
            if globe.DEBUG:
                print("active", self.name, flush=True)
            self._plus_power(player, 1)
    ...
    def play_action(self, player):
        ...
        for c in player.discarded_this_turn:
            total_power += 1
        player.triggers.append(self.trigger)
        self._plus_power(player, total_power)
        return 0
...
class t_spheres(card_frame.card):
    ...
    def play_action(self, player):
        ...
        for c in grand_assemble:
            if c.name not in assemble_names:
        ...
        while len(revealed) > 0:
            result = effects.choose_one_of("Put one of the revealed cards on top of your deck", player, revealed,
                                           ai_hint.WORST)
            player.deck.contents.append(result)
...
class the_hourglass(card_frame.card):
    ...
    def next_turn(self):
        if self.played_by is not None and self.card_choosen.owner == self.played_by and (
                self.card_choosen.find_self()[0] == self.played_by.played
    ...
    def play_action(self, player):
        ...
        for c in player.played.contents:
            if c.cost <= 6 and c != self:
                assemble.append(c)
        if len(assemble) > 0:
            result = effects.choose_one_of("Choose a card to have in your hand next turn.", player, assemble,
                                           ai_hint.BEST)
...
class eclipso(card_frame.card):
    ...
    def special_action_click(self, player):
        ...
        result = False
        while result is not None and len(self.discarded_cards) > 0:
...
class gog(card_frame.card):
    ...
    def stack_ongoing(self, player):
        ...
        if not destroyed_a_card:
            if len(player.hand.contents) > 0:
                result = effects.choose_one_of("Stack Ongoing: Discard a card.", player, player.hand.contents,
                                               ai_hint.WORST)
            ...
            if effects.attack(p, self):
                ...
                for c in p.hand.contents:
                    if c.cost >= 3:
                        assemble.append(c)
                if len(assemble) > 0:
                    result = effects.choose_one_of("Choose a card with cost 3 or greater to discard", p, assemble,
                                                   ai_hint.WORST)
    ...
    def play_action(self, player):
        ...
        if len(player.hand.contents) > 0:
            result = effects.choose_however_many(
                "Choose any number of cards to put on the bottom of your deck.\nYou will draw that many cards.", player,
                player.hand.contents, ai_hint.IFBAD)
            if result != None:
                for c in result:
                    c.pop_self()
