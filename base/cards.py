...
class aquamans_trident(card_frame.card):
    ...
    def play_action(self, player):
        ...
        for c in player.gained_this_turn:
            if c in player.discard.contents and effects.ok_or_no(
                    f"Would you like to put {c.name} on top of your deck?-", player, c, ai_hint.ALWAYS):
                player.deck.add(c.pop_self())
                break
        else: # nobreak
...
class bulletproof(card_frame.card):
    ...
    def defend(self, attacker=None, defender=None):
        ...
        card_to_destroy = effects.may_choose_one_of(instruction_text, self.owner, self.owner.discard.contents,
                                                    ai_hint.IFBAD)
...
class the_dark_knight(card_frame.card):
    ...
    def trigger(self, ttype, data, player, active, immediate):
        ...
        if trigger.test(immediate, trigger.GAIN_CARD, self.trigger, player, ttype) \
                and data[0] == False \
                and effects.ok_or_no(f"Would you like to put {data[1].name} into your hand?", player, data[1],
                                     ai_hint.ALWAYS):
            if globe.DEBUG:
    ...
    def catwoman_catchup(self, player):
        ...
        for c in player.gained_this_turn:
            if c in player.discard.contents:
                assemble.append(c)
        if len(assemble) > 0:
            result = effects.may_choose_one_of("Would you like to put a card you gined this turn, into your hand?",
    ...
    def triggerCW(self, ttype, data, player, active, immediate):
        ...
        if trigger.test(not immediate,
                        trigger.PLAY,
                        self.triggerCW,
                        player, ttype) and data[0].name == "Catwoman":
            if globe.DEBUG:
            ...
            if p != by_player and effects.attack(p, self, by_player):
                ...
                if len(assemble) > 0:
                    result = effects.choose_one_of(
                        "Choose punch or Vunerability from your discard to put on top of your deck", p, assemble,
                        hint=ai_hint.BEST)
...
class nth_metal(card_frame.card):
    ...
    def play_action(self, player):
        ...
        if top_card != None and effects.ok_or_no(
                f"A {top_card.name} is on top of your deck, would you like to destroy it? (ok/no)", player, top_card,
                ai_hint.IFBAD):
...
class the_riddler(card_frame.card):
    ...
    def play_action(self, player):
        ...
        if effects.ok_or_no(f"Would you like to use the riddler ability?", player, self, ai_hint.NEVER):
            player.played.special_options.append(actions.special_action("Riddle", self.special_action_click, self))
        else:
            self._plus_power(player, 1)
        return 0
...
class solomon_grundy(card_frame.card):
    ...
    def trigger(self, ttype, data, player, active, immediate):
        ...
        if trigger.test(immediate,
                        trigger.GAIN_CARD,
                        self.trigger,
                        player, ttype) and data[1] == self and data[0] == False and effects.ok_or_no(
            f"Would you like to put {data[1].name} on top of your deck?", player, data[1], ai_hint.ALWAYS):
            if globe.DEBUG:
                print("active", self.name, flush=True)
            player.deck.contents.append(data[1])
            player.triggers.remove(self.trigger)
            ...
            if p != by_player and effects.attack(p, self, by_player):
                ...
                if card_to_discard != None:
                    ...
                    if not card_to_discard.ctype_eq(CardType.LOCATION):
                        result = effects.ok_or_no(f"Would you like to play a {card_to_discard.name}?", by_player,
                                                  card_to_discard, ai_hint.ALWAYS)
                        if result:
...
class two_face(card_frame.card):
    ...
    def play_action(self, player):
        self._plus_power(player, 1)
        choose_even = effects.choose_even_or_odd(
            "Choose even or odd, then reveal the top card of your deck.  If its cost matches your choice, draw it.  If not, discard it.",
            player)
    ...
    def play_action(self, player):
        ...
        if len(assemble) > 0:
            result = effects.may_choose_one_of(
                "You may play of of the revealed cards.\nIt will then go back ontop of the players deck.", player,
    ...
    def play_action(self, player):
        ...
        for i in range(2):
            ...
            if card_to_destroy:
                ...
                card_to_destroy = effects.may_choose_one_of(instruction_text, player, player.discard.contents,
                                                            ai_hint.IFBAD)
                if card_to_destroy != None:
                    card_to_destroy.destroy(player)
        return 0
    ...
    def first_apearance(self):
        ...
        for p in globe.boss.players:
            ...
            for i in range(2):
                ...
                if p.hand.size() > 0:
                    cards_to_shuffle.append(
                        effects.choose_one_of(instruction_text, p, p.hand.contents, ai_hint.WORST).pop_self())
            ...
            if effects.attack(p, self):
                ...
                if not has_villain:
                    for i in range(2):
                        ...
                        if p.hand.size() > 0:
                            card_to_discard = effects.choose_one_of(instruction_text, p, p.hand.contents,
                                                                    hint=ai_hint.WORST)
                            p.discard_a_card(card_to_discard)
        return
