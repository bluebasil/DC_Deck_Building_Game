...
BACKGROUND_TEXTURE = arcade.load_texture("images/background.jpg")
CPU_TEXTURE = arcade.load_texture("images/cpu_frame_transparent.png")
POINT_TEXTURE = arcade.load_texture("images/pointer.png")
BUTTON_TEXTURE = arcade.load_texture("images/button_thin.png")
CARD_BUTTON_TEXTURE = arcade.load_texture("images/largebutton_new.png")
...
SMALL_BUTTON_3 = arcade.load_texture("images/small_button_red.png")
    ...
    def draw(self, x, y):
        ...
        for i, p in enumerate(globe.boss.players[1:]):
            ...
            if i + 1 == globe.boss.whose_turn:
                arcade.draw_texture_rectangle(400 * SCREEN_SCALE + 100 * SCREEN_SCALE, start - 100 * SCREEN_SCALE,
                                              POINT_TEXTURE.width * point_scale, \
                                              POINT_TEXTURE.height * point_scale, POINT_TEXTURE, 0)
            start -= siz + 25 * SCREEN_SCALE

        if 0 == globe.boss.whose_turn:
            arcade.draw_texture_rectangle(400 * SCREEN_SCALE + 100 * SCREEN_SCALE, 800 * SCREEN_SCALE,
                                          POINT_TEXTURE.width * point_scale, \
                                          POINT_TEXTURE.height * point_scale, POINT_TEXTURE, 90)

        lineup = self.get_drawable(pile, "linup")
        lineup.draw_squished(globe.boss.lineup.contents, SCREEN_WIDTH / 2 + BASE_TEXTURE.width * CARD_SCALE / 2,
                             SCREEN_HEIGHT / 2 + BASE_TEXTURE.height * CARD_SCALE + 15 * SCREEN_SCALE,
                             (BASE_TEXTURE.width * CARD_SCALE + 15 * SCREEN_SCALE) * 5, True, justify="center")
        ...
        for i, p in enumerate(globe.boss.players):
            ...
            play.draw(p.played.contents, SCREEN_WIDTH / 2 + 50 * SCREEN_SCALE, SCREEN_HEIGHT / 2, True,
                      justify="center")
        ...
        # Pause Button
        pause_button = self.get_drawable(custom_button, "pause_button")
        resume_button = self.get_drawable(custom_button, "resume_button")
        skip_button = self.get_drawable(custom_button, "skip_button")

        def pause():
            globe.PAUSED_STATE = 0

        def resume():
            globe.PAUSED_STATE = 1

        def skip():
            globe.PAUSED_STATE = 2

        pause_button.draw(pause, "⏸", SCREEN_WIDTH * 0.21,
                          SCREEN_HEIGHT - 65 * SCREEN_SCALE,
                          SMALL_BUTTON_3 if globe.PAUSED_STATE == 0 else SMALL_BUTTON_1, 0.2, trivial_action=True,
                          hover_texture=None if globe.PAUSED_STATE == 0 else SMALL_BUTTON_2)
        resume_button.draw(resume, "▶", SCREEN_WIDTH * 0.21 + 50 * SCREEN_SCALE,
                           SCREEN_HEIGHT - 65 * SCREEN_SCALE,
                           SMALL_BUTTON_3 if globe.PAUSED_STATE == 1 else SMALL_BUTTON_1, 0.2, trivial_action=True,
                           hover_texture=None if globe.PAUSED_STATE == 1 else SMALL_BUTTON_2)
        skip_button.draw(skip, "⏭", SCREEN_WIDTH * 0.21 + 2 * 50 * SCREEN_SCALE,
                         SCREEN_HEIGHT - 65 * SCREEN_SCALE,
                         SMALL_BUTTON_3 if globe.PAUSED_STATE == 2 else SMALL_BUTTON_1, 0.2, trivial_action=True,
                         hover_texture=None if globe.PAUSED_STATE == 2 else SMALL_BUTTON_2)
...
class player_icon(drawable):
    def draw(self, player, x, y, angle):
        ...
        hight = wid / 1.96
        arcade.draw_texture_rectangle(wid / 2, y - hight / 2 + 7, wid, hight, CPU_TEXTURE, 0)
        ...
        if player.persona != None:
            ...
            MC.draw(player.persona, x + player.persona.texture.width * 0.2 / 2 * SCREEN_SCALE,
                    y - player.persona.texture.height * 0.2 / 2 * SCREEN_SCALE, 0.2)

            text_size = 20
            # text_offset = 5
            if player.vp > 0:
                text_offset = int(math.log(player.vp, 10)) * 6 + 5
                arcade.draw_circle_filled(x + player.persona.texture.width * 0.2 / 2 * SCREEN_SCALE,
                                          y - player.persona.texture.height * 0.2 / 2 * SCREEN_SCALE, 25 * SCREEN_SCALE,
                                          arcade.color.BLUE)
                arcade.draw_text(f"{player.vp}",
                                 x + player.persona.texture.width * 0.2 / 2 * SCREEN_SCALE - text_offset,
                                 y - player.persona.texture.height * 0.2 / 2 * SCREEN_SCALE - text_size * 0.33,
                                 arcade.color.WHITE, text_size)

            discard = self.get_drawable(pile, f"{self.name}-discard")
            if len(player.discard.contents) > 0:
                discard.draw_single(player.discard.contents,
                                    x + player.persona.texture.width * 0.2 * 1.25 * SCREEN_SCALE,
                                    y - BASE_TEXTURE.height * 0.2 * 0.25 * SCREEN_SCALE, 0.2)
            else:
                discard.set_gone()

            deck = self.get_drawable(pile, f"{self.name}-deck")
            if len(player.deck.contents) > 0:
                deck.draw_single_down(player.deck.contents,
                                      x + player.persona.texture.width * 0.2 * 1.25 * SCREEN_SCALE,
                                      y - player.persona.texture.height * 0.2 * SCREEN_SCALE + BASE_TEXTURE.height * 0.2 * 0.25 * SCREEN_SCALE,
                                      0.2)
            else:
                deck.set_gone()
            ...
            if len(player.ongoing.contents) > 0:
                ...
                ongoing.draw_squished(player.ongoing.contents,
                                      x + player.persona.texture.width * 0.2 * 1.75 * SCREEN_SCALE,
                                      y - BASE_TEXTURE.height * 0.2 * 0.25 * SCREEN_SCALE, 150, True, 0.2)

            hand = self.get_drawable(pile, f"{self.name}-hand")
            if len(player.hand.contents) > 0:
                hand.draw_squished(player.hand.contents, x + player.persona.texture.width * 0.2 * 1.75 * SCREEN_SCALE,
                                   y - player.persona.texture.height * 0.2 * SCREEN_SCALE + BASE_TEXTURE.height * 0.2 * 0.25 * SCREEN_SCALE,
                                   150, False, 0.2)
            else:
                hand.set_gone()

            arcade.draw_text(f"Score: {player.score}", x + 400 * SCREEN_SCALE,
                             y - player.persona.texture.height * 0.1 / 2 * SCREEN_SCALE, arcade.color.WHITE, 15)

            on_top = self.get_drawable(pile, f"{self.name}-on_top")
            if len(player.over_superhero.contents) > 0:
                on_top.draw_squished(player.over_superhero.contents,
                                     x + player.persona.texture.width * 0.2 * 3.5 * SCREEN_SCALE,
                                     y - player.persona.texture.height * 0.2 * SCREEN_SCALE + BASE_TEXTURE.height * 0.2 * 0.25 * SCREEN_SCALE,
                                     150, True, 0.2)
            else:
                on_top.set_gone()
...
class player(drawable):
    def draw(self, player, x, y, angle):
        ...
        if player.persona != None:
            ...
            if player.vp > 0:
                text_offset = int(math.log(player.vp, 10)) * 6 + 5
                arcade.draw_circle_filled(x + player.persona.texture.width * SCREEN_SCALE / 2, self.maxy / 2, 50,
                                          arcade.color.BLUE)
                arcade.draw_text(f"{player.vp}", x + player.persona.texture.width * SCREEN_SCALE / 2 - text_offset,
                                 self.maxy / 2 - text_size * 0.33, arcade.color.WHITE, text_size)

            on_top = self.get_drawable(pile, "on_top")
            if len(player.over_superhero.contents) > 0:
                on_top.draw_squished(player.over_superhero.contents,
                                     x + player.persona.texture.width * 0.25 * SCREEN_SCALE,
                                     self.maxy - BASE_TEXTURE.height * CARD_SCALE * 0.5 / 2, 200 * SCREEN_SCALE, True,
                                     scale=0.5)
            else:
                on_top.set_gone()

            under = self.get_drawable(pile, "under")
            if len(player.under_superhero.contents) > 0:
                under.draw_squished(player.under_superhero.contents,
                                    x + player.persona.texture.width * 0.25 * SCREEN_SCALE,
                                    self.miny + BASE_TEXTURE.height * CARD_SCALE * 0.5 / 2, 200 * SCREEN_SCALE, False,
                                    scale=0.5)
            else:
                under.set_gone()

            # Score should be written over the other card piles
            arcade.draw_text(f"Score: {player.score}",
                             x + 15 * SCREEN_SCALE,
                             self.maxy + 60 * SCREEN_SCALE, arcade.color.WHITE, 15)
...
class scroller_right(drawable):
    ...
    def draw(self, pile_parent, x, y):
        ...
        self.set_juristiction(x + width / 2 - 15 * SCREEN_SCALE / 2, y - height / 2, x + width + 15 * SCREEN_SCALE / 2,
                              y + height / 2)
...
class pile(drawable):
    ...
    def ready_card(self, c, x, y, i, depth=0, scale=1):
        if c == option.OK or c == option.NO or c == option.DONE or c == option.EVEN or c == option.ODD:
            ...
            new_card.draw(c, x, y, scale)
            new_card.depth = depth

    def draw(self, pile_contents, x, y, center=False, justify="right", width=1500 * SCREEN_SCALE):
        self.draw_squished(pile_contents, x, y, width, True, justify=justify)
    ...
    def draw_squished(self, pile_contents, x, y, width, visible, scale=1, justify="left"):
        # For debug purposes
        # self.single = True
        super().draw()
        ...
        if len(pile_contents) > 0:
            separation = min(width / len(pile_contents), BASE_TEXTURE.width * CARD_SCALE + 15 * SCREEN_SCALE)
        pos = x
        if justify == "center":
            pos -= width / 2
        elif justify == "right":
        ...
        for i, c in enumerate(pile_contents):
            ...
            if visible:
                self.ready_card(c, pos, y, i, depth=self.depth - i, scale=scale)
                # new_card.draw(c, pos, y, scale)
            else:
                new_card = self.get_drawable(card, f"{i}")
                new_card.draw_down(pos, y, scale)
                new_card.depth = self.depth - i - 1
            pos += separation
...
class button(drawable):
    ...
    def draw(self, action, text, x, y, card_size=False, trivial_action=False):
        super().draw()
        self.action = action
        self.trivial_action = trivial_action
        # print(card.name)
        width = BASE_TEXTURE.width * CARD_SCALE * 0.9
        height = width / 2  # Used to be the golden ratio: 1.61
    ...
    def mouse_up(self, mouse, x, y):
        ...
        if not mouse.consumed and self.check_collision(x, y):
            ...
            if self.trivial_action:
                self.action()
            elif not mouse.silent and not self.silent:
                globe.bus.button_clicked(self.action)


class custom_button(drawable):
    hover = False

    def draw(self, action, text, x, y, texture, scale, trivial_action=False, hover_texture=None):
        super().draw()
        self.action = action
        self.trivial_action = trivial_action
        ...
        arcade.draw_texture_rectangle(x, y, width, height,
                                      texture if not self.hover or hover_texture is None else hover_texture, 0)
        arcade.draw_text(f"{text}", x - text_offset, y - text_size / 3, arcade.color.WHITE, text_size)
    ...
    def mouse_move(self, preview, x, y):
        if not preview.consumed and self.check_collision(x, y):
            self.hover = True
        elif preview.consumed or not self.check_collision(x, y):
            self.hover = False
...
class question(drawable):
    ...
    def draw(self, question, x, y):
        ...
        choices.draw(question.options, x, y - height * 0.25, True, justify="center")
