...
class broadsword(card_frame.card):
    ...
    def attack_action(self, by_player):
        ...
        player = effects.choose_a_player("Choose a player to destroy a cost 1,2, or 3 card from their discard",
                                         by_player, False)
        if effects.attack(player, self, by_player):
            ...
            for c in player.discard.contents:
                if 1 <= c.cost <= 3:
                    assemble.append(c)
            if len(assemble) > 0:
                result = effects.choose_one_of(f"destroy a cost 1,2, or 3 card from {player.persona.name}'s' discard.",
                                               by_player, assemble, hint=ai_hint.BEST)
            ...
            if p != by_player and effects.attack(p, self, by_player):
                for i in range(2):
                    if len(p.hand.contents) > 0:
                        result = effects.choose_one_of(f"Choose a card to discard. ({i + 1}/2)", p, p.hand.contents,
                                                       ai_hint.WORST)
...
class despero(card_frame.card):
    ...
    def attack_action(self, by_player):
        player = effects.choose_a_player("Choose a player to force to discard a cards costing 1 or greater.", by_player,
                                         False)
        if effects.attack(player, self, by_player):
            assemble = []
            for c in player.hand.contents:
...
class dr_light(card_frame.card):
    ...
    def attack_action(self, by_player):
        player = effects.choose_a_player("Choose a player to force to discard a location they control", by_player,
                                         includes_self=False, hint=ai_hint.RANDOM)
        if effects.attack(player, self, by_player):
            assemble = []
            ...
            if drawn.cost <= 5:
                ...
                if effects.ok_or_no(instruction_text, player, drawn, hint=ai_hint.IFGOOD):
                    ...
                    def replace_play_action(player, actual_self=drawn):
                        ...
                        actual_self.action = actions.special_action(f"Firestorm-{actual_self.name}",
                                                                    actual_self.firestorm_special_action_click, actual_self)
...
class insanity(card_frame.card):
        ...
        if len(player.played.played_this_turn) == 0:
            ...
            for i, p in enumerate(globe.boss.players):
                ...
                if current is not None:
                    ...
                    player.card_has_been_passed(current)
        return 0
    ...
    def defend(self, defender=None, attacker=None):
        ...
        if attacker != None:
            assemble = []
            assemble.extend(defender.hand.contents)
            assemble.extend(defender.discard.contents)
            if len(assemble) > 0:
                instruction_text = f"You may put a card from your hand or discard pile into {attacker.persona.name}'s hand"
                result = effects.may_choose_one_of(instruction_text, defender, assemble, ai_hint.WORST)
                if result != None:
                    result.pop_self()
    ...
    def play_action(self, player):
        ...
        if top_card != None:
            ...
            if effects.ok_or_no(instruction_text, player, top_card, hint=ai_hint.IFGOOD):
                ...
                player.card_has_been_passed(top_card)
...
class man_bat_serum(card_frame.card):
    ...
    def play_action(self, player):
        ...
        self._plus_power(player, player.vp)
        return 0

    def end_of_turn(self):
        if self.destroy_by is not None:
...
class owlman(card_frame.card):
    ...
    def play_action(self, player):
        ...
        self._plus_power(player, len(unique_equipment))
        return 0
...
class pandora(card_frame.card):
    ...
    def play_action(self, player):
        ...
        self._plus_power(player, len(unique_cost))
        return 0
...
class power_ring(card_frame.card):
    ...
    def play_action(self, player):
        ...
        self._plus_power(player, len(unique_hero))
        return 0
...
class superwoman(card_frame.card):
    ...
    def play_action(self, player):
        ...
        self._plus_power(player, len(unique_villain))
        return 0
        ...
        if len(assemble) > 0:
            ...
            for c in globe.boss.lineup.contents:
                if c.cost <= result.cost:
                    assemble.append(c)
            result = effects.may_choose_one_of(f"you may gain a card from the lineup of cost {result.cost} or less",
                                               player, assemble, ai_hint.BEST)
            if result != None:
                player.gain(result)
        return 0
...
class ultraman(card_frame.card):
    ...
    def play_action(self, player):
        ...
        self._plus_power(player, len(unique_superpower))
        return 0
...
class word_of_power(card_frame.card):
    ...
    def destroy(self, player_responsible):
        player_responsible.triggers.append(self.trigger)
        super().destroy(player_responsible)
...
class belle_reve(card_frame.card):
    ...
    def trigger(self, ttype, data, player, active, immediate):
        if globe.DEBUG:
            print("test", self.name, flush=True)
        if trigger.test(not immediate, trigger.PLAY, self.trigger, player, ttype) and data[0].ctype_eq(
                CardType.VILLAIN):
            if globe.DEBUG:
                print("active", self.name, flush=True)
            self._plus_power(player, 1)
...
class central_city(card_frame.card):
    ...
    def trigger(self, ttype, data, player, active, immediate):
        ...
        if trigger.test(not immediate,
                        trigger.PLAY,
                        self.trigger,
                        player, ttype) and data[0].ctype_eq(CardType.SUPERPOWER) and data[0].name != "Kick":
            if globe.DEBUG:
                print("active", self.name, flush=True)
            self._plus_power(player, 1)
...
class earth_3(card_frame.card):
    ...
    def play_action(self, player):
        if self not in player.ongoing.contents:
            player.ongoing.add(self.pop_self())
        # player.played.card_mods.append(self.location_mod)
        self.action = actions.special_action("Earth-3", self.special_action_click, self)
        player.played.special_options.append(self.action)
        return 0
...
class happy_harbor(card_frame.card):
    ...
    def trigger(self, ttype, data, player, active, immediate):
        ...
        if trigger.test(not immediate,
                        trigger.PLAY,
                        self.trigger,
                        player, ttype) and data[0].ctype_eq(CardType.HERO):
            if globe.DEBUG:
                print("active", self.name, flush=True)
            self._plus_power(player, 1)
    ...
    def play_action(self, player):
        ...
        if len(assemble) > 0:
            result = effects.choose_one_of(
                "Choose one of these to destroy.  You will gain the destroyed costs VP value.", player, assemble,
                ai_hint.WORST)
...
class cyborg(card_frame.card):
    ...
    def trigger(self, ttype, data, player, active, immediate):
        ...
        if trigger.test(not immediate,
                        ...
                        self.trigger,
                        player, ttype) and (
                data[0].ctype_eq(CardType.SUPERPOWER) or data[0].ctype_eq(CardType.EQUIPMENT)):
            if globe.DEBUG:
                print("active", self.name, flush=True)
            self._plus_power(player, 2)
    ...
    def play_action(self, player):
        ...
        for c in player.played.played_this_turn:
            if c.ctype_eq(CardType.SUPERPOWER) \
                ...
                so_far_power += 2
        self._plus_power(player, so_far_power)
        player.triggers.append(self.trigger)
        return 0
...
class martian_manhunter(card_frame.card):
    ...
    def trigger(self, ttype, data, player, active, immediate):
        ...
        if trigger.test(not immediate,
                        trigger.PLAY,
                        self.trigger,
                        player, ttype) and (data[0].ctype_eq(CardType.HERO) or data[0].ctype_eq(CardType.VILLAIN)):
            if globe.DEBUG:
                print("active", self.name, flush=True)
            self._plus_power(player, 2)
...
class shazam(card_frame.card):
    ...
    def play_action(self, player):
        ...
        if len(assemble) > 0:
            result = effects.choose_one_of("Destroy one of the cards that you just gained.", player, assemble,
                                           ai_hint.WORST)
            result.destroy(player)
        return 0
            ...
            if effects.attack(p, self):
                ...
                for c in p.discard.contents:
                    if c.cost >= 5:
                        assemble.append(c)
                if len(assemble) > 0:
                    result = effects.choose_one_of("Choose a card to put on the bottom of the main deck.", p, assemble,
                                                   ai_hint.WORST)
                    result.set_owner(owners.MAINDECK)
                    result.pop_self()
                    globe.boss.main_deck.contents.insert(0, result)
            ...
            if effects.attack(p, self):
                ...
                for c in p.discard.contents:
                    if c.ctype_eq(CardType.SUPERPOWER):
                        assemble.append(c)
                if len(assemble) > 0:
                    result = effects.choose_one_of("Destroy a Super Power from your hand or discard pile", p, assemble,
                                                   ai_hint.WORST)
                    result.destroy(p)
                else:
                    for i in range(2):
    ...
    def play_action(self, player):
        ...
        for p in globe.boss.players:
            for c in p.ongoing.contents:
                if c.ctype_eq(CardType.LOCATION):
                    locations_in_play += 1
        self._plus_power(player, locations_in_play * 2)
        return 0
            ...
            if effects.attack(p, self):
                ...
                for c in p.discard.contents:
                    if c.ctype_eq(CardType.VILLAIN):
                        assemble.append(c)
                if len(assemble) > 0:
                    result = effects.choose_one_of("Destroy a Villain from your hand or discard pile", p, assemble,
                                                   ai_hint.WORST)
                    result.destroy(p)
                else:
                    p.gain_a_weakness()
