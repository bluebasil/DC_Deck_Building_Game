        ...
        for p in globe.boss.players:
            if p != by_player and effects.attack(p, self, by_player):
                if len(p.hand.contents) > 0:
                    instruction_text = f"Reveal a card to put under your Super Hero.\nIf one or more foes does not " \
                                       f"reveal a card with cost 1 or\ngreater, {by_player.persona.name} will draw two " \
                                       f"cards. "
...
class explosive_arrow(card_frame.card):
    ...
    def attack_action(self, by_player):
        p = effects.choose_a_player("Choose a player.\nYou will destroy a card under that players Super Hero.",
                                    by_player, includes_self=False, hint=ai_hint.BEST)
        if effects.attack(p, self, by_player):
            if len(p.under_superhero.contents) > 0:
                result = effects.choose_one_of(f"Choose a card to destroy from under {p.persona.name}'s Superhero",
...
class huntress(card_frame.card):
    ...
    def trigger(self, ttype, data, player, active, immediate):
        if trigger.test(not immediate,
            ...
            if len(assemble) > 0:
                result = effects.may_choose_one_of("You may destroy a card in your hand or discard pile.", player,
                                                   assemble, ai_hint.IFBAD)
                if result != None:
        ...
        if len(player.hand.contents) > 0:
            ...
            if result != None:
                ...
                if len(player.under_superhero.contents) > 0:
                    ...
                    result = effects.may_choose_one_of(instruction_text, player, player.under_superhero.contents,
                                                       ai_hint.BEST)
                    if result != None:
                        result.pop_self()
...
class mr_blank(card_frame.card):
    ...
    def attack_action(self, by_player):
        p = effects.choose_a_player(
            "Choose a player.\nYou will put a card from under their Super Hero, under your Super Hero", by_player,
            includes_self=False, hint=ai_hint.BEST)
        if effects.attack(p, self, by_player):
            if len(p.under_superhero.contents) > 0:
                result = effects.choose_one_of(f"Put one of these cards from {p.persona.name} under your Super Hero.",
                                               by_player, p.under_superhero.contents, ai_hint.BEST)
                result.pop_self()
...
class you_have_failed_this_city(card_frame.card):
    ...
    def attack_action(self, by_player):
        p = effects.choose_a_player("Choose a player.\nThey will discard a random card.", by_player,
                                    includes_self=False, hint=ai_hint.BEST)
        if effects.attack(p, self, by_player):
            if len(p.hand.contents) > 0:
                ...
                if not top_card.ctype_eq(cardtype.VILLAIN):
                    ...
                    for c in p.discard.contents:
                        if c.cost <= 3:
                            assemble.append(c)
                    if len(assemble) > 0:
                        result = effects.choose_one_of("Choose a card with cost 3 or less to discard.", p, assemble,
                                                       ai_hint.WORST)
                        result.destroy(p)
        return
